...
pub async fn main(req: Request, env: Env) -> Result<Response> {
    ...
    router
        ...
        .get("/uuid", |_, _| {
            let uuid_string = Uuid::new_v4();

            Response::ok(uuid_string.to_string())
        })
        .get("/uuid/:count", |_, ctx| {
            let uuid_string: String;
            let count_param = ctx.param("count");

            if let Some(count) = count_param {
                let count: i32 = count.parse().unwrap();
                uuid_string = multiple_uuids(count);
                Response::ok(uuid_string)
            } else {
                Response::error("Need count", 400)
            }
        })
